...
{
    /// <summary>
    /// Event 타입 카드의 능력(Ability) 정의를 담는 상세 테이블 행(row) 데이터입니다.
    ...
    public sealed class StruckTableTcgCardEvent : ITcgCardAbilityRow
    {
        ...
        /// <summary>이벤트 카드가 가지는 능력의 종류입니다.</summary>
        ...
        /// <summary>
        /// 트리거 발동 후 이 카드를 소모(묘지로 이동)할지 여부입니다.
        /// </summary>
        public bool consumeOnTrigger;
    }

    /// <summary>
    /// Event 타입 카드의 능력(Ability) 상세 테이블을 로드하는 테이블 클래스입니다.
    /// 로드된 행 데이터는 Uid를 기준으로 기본 카드 테이블과 결합되어 사용됩니다.
    /// </summary>
    public sealed class TableTcgCardEvent : DefaultTable<StruckTableTcgCardEvent>
    {
        /// <summary>
        /// Addressables(또는 테이블 로더)에서 이벤트 카드 테이블을 식별하기 위한 키입니다.
        /// </summary>
        public override string Key => ConfigAddressableTableTcg.TcgCardEvent;

        /// <summary>
        /// 테이블 한 행의 원시(Dictionary) 데이터를 <see cref="StruckTableTcgCardEvent"/>로 변환합니다.
        /// 능력 타입/트리거/타겟과 파라미터(A/B/C), 그리고 소모 여부를 파싱합니다.
        /// </summary>
        /// <param name="data">컬럼명-문자열 값 형태의 원시 행 데이터입니다.</param>
        /// <returns>변환된 이벤트 카드 능력 행 데이터입니다.</returns>
        /// <exception cref="KeyNotFoundException">
        /// 필수 컬럼(Uid, AbilityType, TriggerType, TargetType, ParamA, ParamB, ParamC, ConsumeOnTrigger)이 누락된 경우 발생할 수 있습니다.
        /// </exception>
